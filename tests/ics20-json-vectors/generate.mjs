...
const IBC_RS_V053_SOURCE = {
  repository: "github.com/informalsystems/ibc-rs",
  version: "v0.53.0",
  serializer: {
  ...
  sendPath:
    "ibc-apps/ics20-transfer/src/handler/send_transfer.rs::serde_json::to_vec",
  fixture:
    "ibc-apps/ics20-transfer/types/src/packet.rs::dummy_json_packet_data",
};
const IBC_RS_V053_UPSTREAM_FIXTURE = {
  name: "ibc_rs_v0_53_upstream_fixture",
  json:
    '{"denom":"uatom","amount":"10","sender":"cosmos1wxeyh7zgn4tctjzs0vtqpc6p5cxq5t2muzl7ng","receiver":"cosmos1wxeyh7zgn4tctjzs0vtqpc6p5cxq5t2muzl7ng","memo":""}',
};
const GO_GENERATORS = [
  {
    profile: "ibcGoV8",
...
function quoteIbcRsString(value) {
  let quoted = '"';
...
function ibcRsV053JSON(data) {
  const fields = ["denom", "amount", "sender", "receiver", "memo"].map(
    (field) => `${JSON.stringify(field)}:${quoteIbcRsString(data[field])}`,
  );
  return `{${fields.join(",")}}`;
}
...
function combineVectors(cases, generatedByProfile) {
  const vectors = cases.map((testCase, index) => {
    const wire = {
      ...
      ibcRsV053: wireEncoding(ibcRsV053JSON(testCase.data)),
    };
  ...
  const upstreamFixture = vectors.find(
    ({ name }) => name === IBC_RS_V053_UPSTREAM_FIXTURE.name,
  );
  if (
    upstreamFixture?.wire.ibcRsV053.json !==
    IBC_RS_V053_UPSTREAM_FIXTURE.json
  ) {
    throw new Error(
      `${IBC_RS_V053_UPSTREAM_FIXTURE.name}.ibcRsV053 no longer matches ` +
        `${IBC_RS_V053_SOURCE.version} ${IBC_RS_V053_SOURCE.fixture}`,
    );
  }
...
function renderAiken(vectors) {
  const lines = [
    ...
    "// The Cosmos bytes come from ibc-go GetBytes() and the pinned ibc-rs v0.53 fixture/profile.",
  ...
  for (const vector of vectors) {
    lines.push(
      ...
      "}",
      "",
      `test pinned_${vector.name}_ibc_rs_v0_53_bytes() {`,
      "  let expected =",
      `    #"${vector.wire.ibcRsV053.hex}"`,
      "  and {",
      `    packet_data.get_ibc_rs_bytes(${vector.name}()) == expected,`,
      `    packet_data.matches_bytes(${vector.name}(), expected),`,
      "  }",
      "}",
      "",
