...
For a new baseline, set `IBC_DEPLOYMENT_MODE=upgradeable` and supply `MIGRATION_GOVERNANCE_FILE` to the existing `cardano/offchain/index.ts` deployment entry point. Its JSON contains explicit `signers` (distinct payment-key hashes), `quorum`, and `delay_ms` (at least `86400000`), with numeric fields encoded as decimal strings, plus `emergency: {"signers": [...], "quorum": "..."}` using a disjoint signer set. Missing/unknown deployment mode, missing governance in upgradeable mode, invalid governance, or governance supplied in legacy mode fails before provider access or transaction submission. Intentional immutable deployment requires `IBC_DEPLOYMENT_MODE=legacy` explicitly. New artifacts carry `deploymentMode`; Gateway defaults to requiring recovery for unlabelled artifacts, so intentional older immutable artifacts require explicit `IBC_DEPLOYMENT_MODE=legacy` at startup. The SDK requires an explicit legacy manifest marker. A label alone does not establish recovery: readiness reads the canonical registry NFT and checks identity, immutable state policies, all five full role addresses, generation and compatibility, then resolves/proves current HostState. The standard single-wallet entry point supports a bootstrap authority signed by that explicitly configured wallet. Threshold bootstrap uses `createDeployment` with `deploymentMode: "upgradeable"`, an explicit `bootstrapSigners` quorum and `signRegistryBootstrap` callback that returns the fully signed transaction; missing signing support fails before deployment. Later approvals can use the unsigned export workflow below. Never substitute rehearsal authority keys for production custody.
...
The output is created exclusively. Preserve its reported canonical-JSON SHA-256, not just a filename. Preparation does not authorize a transition. Preparation outrefs are provenance, not durable constraints. Ordinary HostState/transfer-root continuations do not require new review. Authorization resolves current authenticated NFTs and rechecks inventory/count limits. Changed governance nonce, source generation, inventory or successor configuration requires fresh review; `Begin` independently verifies the current state against the on-chain proposal. Ordinary traffic racing approval can still change inventory and prevent `Begin`, requiring cancellation and a new proposal. Approval transactions no longer reference the volatile preparation objects.
...
## Recovery duration and inventory cache

All object moves and activation serialize through the registry UTxO. For `C` clients, `L` connections, `H` channels and `N` escrow shards, handover requires `C + L + H + N + 3` transactions: Begin/HostState, each core object, transfer root, each escrow, Activate. Approval and script publications are additional. Ordinary operations stop at Begin and resume only after activation plus operational manifest installation; they remain available during the approval delay except object creation. At a hypothetical 20 seconds per confirmed transaction, 1,000 shards plus ten core objects take about 5.63 hours for handover. That is an extrapolation, not a measured network SLA. No inclusion deadline is guaranteed.

The operator calls `awaitTx` and checks canonical output adoption; it does **not** wait a configured security-parameter depth or establish irreversible finality. Rollback can require replay/reconstruction, and public-network recovery latency remains unmeasured. Coordinate a confirmation/finality policy before production use; do not infer one from the local single-step budget results.
...
The full rehearsal now requests `--exercise-rollback`: partition the owned five-pool devnet, include one move on the minority branch, wait past its validity interval, reconnect and require ledger-observed rollback before production resume. It records per-command wall time and canonical Begin-to-Activate slots/blocks. This passed on the profile-v3 artifacts at `79dc91f53`: nine canonical custody transactions per handover, 718 seconds with rollback for V2 and 178 seconds for V3. See [source-bound evidence](migration-implementation-evidence.md#scenarios-demonstrated-on-the-local-devnet). These are two-channel/two-shard local-network measurements, not public-network finality or a production-size downtime envelope. Changed compiled artifacts require another run.
