...
Measurements use distinct native denominations assigned to authenticated channels; these historical results are superseded by the latest measurements in the validation summary. At 128 shards uncached construction took 7.782s and evaluation/submission 3.100s; cached construction 2.165s and evaluation/submission 3.093s. Maximum signed handover transaction was 4,136 bytes. These measurements use Emulator default protocol parameters; separate compiled budget checks use the retained protocol-10 cost profile.
...
The cache verifies every constructed witness against a freshly inspected registry commitment. Pending deletion only becomes local progress on observing its exact predicted root. Retry with unchanged root repeats the same valid witness. Other progress/rollback resets from provider inventory and verifies its complete root. Independent-root tests cover rejected/unconfirmed submission, competing removal, restart, stale inventory, duplicate inventory and rollback. These cache/provider tests are deterministic simulations. The subsequent profile-v3 rehearsal at `79dc91f53` also demonstrated an actual five-node minority-fork rollback and canonical recovery; see [its source-bound record](migration-implementation-evidence.md#scenarios-demonstrated-on-the-local-devnet). That does not qualify later changed compiled artifacts.
...
Immediate emergency restriction was not implemented at this historical revision. It is now implemented separately; see the [current containment specification](emergency-containment-audit.md).

## Candidate acceptance

See the [validation summary](migration-implementation-evidence.md) for tested revisions, tool versions, executed scenarios and current results. This historical follow-up is not acceptance of later compiled contracts. The completed profile-v3 devnet rehearsal and the latest candidate's pending network run are identified separately there.
