...
2. True sequential chaining: `X_i` has to consume an output created by the `X_{i-1}` transaction (like `X_3` must consume an output created by `X_2`), which restores the unique-predecessor property. Because a transaction's output reference and datum are fixed by its body, a single builder can pre-construct and sign a topologically ordered chain of dependent transactions before any predecessor is on-chain. This can pipeline several HostState transactions into one block, but it does not provide independent parallelism: multiple relayers cannot each construct and submit their own lane transactions concurrently and expect the chain to order them into the correct monotonically ascending series of shards, **or**
