...
The fresh job in [35253843385](https://github.com/cardano-foundation/cardano-ibc-incubator/actions/runs/35253843385), at `cac3776d0`, stopped before funding submission: Linux could not copy the root-owned mode-0600 public signed envelope from the bind mount. Commit `feadea9c5` reads that public envelope through the owned container into a runner-owned evidence file. It does not change signing-key or directory permissions. Independent read-only review found no additional concrete defect in this startup path: all producers inherit the persistent relative clock, service augmentation preserves node configuration, and exact funding/reference checks precede Gateway startup. These checks establish current ledger membership, not irreversible finality. Fresh full Linux acceptance remains pending.
