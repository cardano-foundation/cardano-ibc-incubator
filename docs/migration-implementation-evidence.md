...
The populated local V1→V2→V3 rehearsal **passed**, including all old claims, new traffic and final canonical revalidation. Fresh CI attempts have failed before population, first resolving a reference and later waiting for disposable faucet funding; startup corrections and diagnostics are being verified. This is not a production-release or audit claim. No mainnet deployment, live authority change, or real user funds are involved. See the [requirement mapping](migration-requirements.md), [design](state-preserving-redeployment.md), and [operator runbook](migration-operator-runbook.md).
