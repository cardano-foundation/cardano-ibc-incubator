...
The populated local V1→V2→V3 rehearsal **passed**, including all old claims, new traffic and final canonical revalidation. The latest fresh CI rehearsal failed during Gateway startup before population; its startup fixes are awaiting a fresh run. This is not a production-release or audit claim. No mainnet deployment, live authority change, or real user funds are involved. See the [requirement mapping](migration-requirements.md), [design](state-preserving-redeployment.md), and [operator runbook](migration-operator-runbook.md).
