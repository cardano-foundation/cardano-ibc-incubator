...
That fresh job stopped before funding submission: Linux could not copy the root-owned mode-0600 public signed envelope from the bind mount. Commit `feadea9c5` reads that public envelope through the owned container into a runner-owned evidence file. It does not change signing-key or directory permissions. Independent read-only review found no additional concrete defect in this startup path: all producers inherit the persistent relative clock, service augmentation preserves node configuration, and exact funding/reference checks precede Gateway startup. These checks establish current ledger membership, not irreversible finality. Fresh run [35257335163](https://github.com/cardano-foundation/cardano-ibc-incubator/actions/runs/35257335163) is checking `ecae7f65f`. Its fresh V1 deployment, including funding and exact publication checks, passed; the populated two-chain stage is running, and full acceptance remains pending.
