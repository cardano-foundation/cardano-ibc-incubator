...
while existing channels may still close, drain, and be reclaimed.
...
Because voucher supply is currently tracked for the transfer module as a whole,
a transfer channel is reclaimed only after that module-wide supply also reaches
zero. This is conservative, but it prevents a channel from disappearing before
wallet-held vouchers can take their normal return path.
