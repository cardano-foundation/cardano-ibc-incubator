...
type DeploymentConfig = {
  ...
  validators: {
    ...
    mintLifecycleCreationMarker: { refUtxo: RefUtxo; scriptHash: string };
    mintLifecycleReclamationMarker: { refUtxo: RefUtxo; scriptHash: string };
    mintLifecycleOperationalMarker: { refUtxo: RefUtxo; scriptHash: string };
    mintLifecyclePacketMarker: { refUtxo: RefUtxo; scriptHash: string };
...
function hashSha3_256Hex(data: string): string {
  return sha3_256(Buffer.from(data, 'hex'));
}
...
async function encodeChannelDatum(
  channelDatum: any,
  Lucid: typeof import('@lucid-evolution/lucid'),
) {
  ...
  if (!CML) {
    ...
    return Data.to(channelDatum, ChannelDatumSchema as any);
  }
  ...
  if (stateIndex === undefined || orderIndex === undefined) {
    throw new Error(
      `Invalid channel datum state/order: state=${String(channelDatum.state.channel.state)}, order=${String(
        channelDatum.state.channel.ordering,
      )}`,
    );
  }
...
function unknownCodecTypeError(operation: 'encode' | 'decode', type: string, supportedTypes: readonly string[]) {
  return new Error(
  ...
  const IBCModuleOperatorSchema = Data.Enum([
    Data.Object({
      ModuleOperatorV1: Data.Tuple([
        (moduleOperatorSchema ?? Data.Any()) as never,
      ]),
    }),
...
async function encodeIbcModuleRedeemer(
  ...
  return Lucid.Data.to(data, schema as any, { canonical: true });
}

async function encodeTransferIbcModuleRedeemer(
  data: any,
  Lucid: typeof import('@lucid-evolution/lucid'),
) {
  const { Data } = Lucid;
  ...
  const schema = ibcModuleRedeemerSchema(
    Lucid,
    FungibleTokenPacketDatumSchema,
    TransferModuleRedeemerSchema,
  );
  return Data.to(data, schema as any, { canonical: true });
}
...
function encodeTransferEscrowShardRedeemer(
  ...
  return Data.to(data, TransferEscrowShardRedeemerSchema as any, {
    canonical: true,
  });
}
...
export class LucidIbcAdapter {
  ...
  private async loadReferenceScripts(): Promise<ReferenceScripts> {
    const outRefs: Record<keyof ReferenceScripts, RefUtxo> = {
      ...
      mintLifecyclePacketMarker:
        this.deployment.validators.mintLifecyclePacketMarker.refUtxo,
    };
  ...
  private hostStateContinuationAssets(
    hostStateUtxo: Pick<UTxO, 'assets'>,
    mintedMarkerUnit?: string,
  ): Record<string, bigint> {
  ...
  private authorizePacketHostTransition(tx: TxBuilder): string {
    const markerUnit =
      this.deployment.validators.mintLifecyclePacketMarker.scriptHash +
  ...
  public createUnsignedSendPacketBurnTx(dto: any): TxBuilder {
    ...
    tx.readFrom([
      ...
      .pay.ToContract(
        this.deployment.modules.transfer.address,
        { kind: 'inline', value: dto.encodedUpdatedTransferModuleDatum },
        dto.transferModuleReferenceUtxo.assets,
      )
      .mintAssets(
