...
import { recordFromConstr } from './consensusHistory';
...
type Match = {
  ...
  output_index: number;
  address: string;
  value: { assets: Record<string, string | number> };
  datum_hash: string | null;
  datum_type?: string;
  created_at: { slot_no: number; header_hash: string };
};

function fields(value: Data, count: number): Data[] {
  if (!(value instanceof Constr) || value.index !== 0 || value.fields.length !== count) {
    throw new Error('Invalid production client datum in Kupo history');
  }
  return value.fields;
}

function entry(value: Data): [Data, Data] {
  if (!(value instanceof Map) || value.size !== 1) throw new Error('Historical client must contain one checkpoint');
...
export function createKupoConsensusHistoryReader(
  ...
  const get = async (path: string): Promise<unknown> => {
    ...
    if (!response.ok) throw new Error(`Kupo consensus history request failed (${response.status})`);
    return response.json();
  };
  return async (client: IbcTreeUtxo) => {
    if (!client.datum || !client.address) throw new Error('Client output is missing its datum or address');
    const [currentState, token] = fields(Data.from(client.datum), 3);
    const [policy, name] = fields(token, 2);
    if (typeof policy !== 'string' || typeof name !== 'string') throw new Error('Invalid client authentication token');
    const unit = policy + name;
    if (client.assets[unit] !== 1n) throw new Error('Client authentication token is missing');
    const [latestHeight] = entry(fields(currentState, 4)[1]);
    const latestKey = Data.to<Data>(latestHeight, undefined, { canonical: true });
    ...
    if (!Array.isArray(response)) throw new Error('Invalid Kupo consensus history response');
    const matches = response as Match[];
    const records = new Map<string, Awaited<ReturnType<NonNullable<IbcTreeLucidService['consensusHistoryRecords']>>>[number]>();
    ...
    for (const match of matches) {
      ...
      if (refs.has(ref)) throw new Error('Duplicate client output in Kupo history');
      refs.add(ref);
      ...
      const datum = await get(`datums/${match.datum_hash}`) as { datum?: unknown } | null;
      if (!datum || typeof datum.datum !== 'string') throw new Error('Kupo has pruned a required historical client datum');
      const [state, historicalToken] = fields(Data.from(datum.datum), 3);
      const [historicalPolicy, historicalName] = fields(historicalToken, 2);
      if (historicalPolicy !== policy || historicalName !== name) throw new Error('Historical client token does not match');
      ...
      const key = Data.to<Data>(height, undefined, { canonical: true });
      if (key !== Data.to<Data>(timeKey, undefined, { canonical: true }) || key !== Data.to<Data>(heightKey, undefined, { canonical: true })) {
        throw new Error('Historical checkpoint processing metadata is misaligned');
      }
      const record = recordFromConstr(new Constr(0, [historicalToken, height, consensusState, time, processedHeight]));
      ...
      if (match.transaction_id === client.txHash && match.output_index === client.outputIndex) {
        if (datum.datum !== client.datum) throw new Error('Kupo current client datum differs from the live input');
        foundCurrent = true;
      }
    }
    if (!foundCurrent) throw new Error('Kupo consensus history has not reached the live client output');
