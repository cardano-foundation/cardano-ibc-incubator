...
function canonicalValidity(invalid: unknown): true {
  ...
  if (invalid !== false) {
    throw new Error(
      "Yaci canonical transaction validity is missing or invalid",
    );
  }
  return true;
}
...
export async function discoverYaciHistoryBootstrap(
  ...
  try {
    ...
    const evidence: HistoryTransaction = {
