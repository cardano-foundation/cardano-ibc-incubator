...
exports.ConsensusHistoryRecovery = exports.HistorySnapshotChangedError = exports.HistoryIntersectionError = void 0;
exports.validateHistoryBootstrap = validateHistoryBootstrap;
const lucid_1 = require("@lucid-evolution/lucid");
const node_sqlite_1 = require("node:sqlite");
const node_buffer_1 = require("node:buffer");
const blake2b_1 = require("@noble/hashes/blake2b");
const cbor_1 = require("@harmoniclabs/cbor");
const consensusHistory_ts_1 = require("./consensusHistory.js");
const incrementalIbcTree_ts_1 = require("./incrementalIbcTree.js");
const plutusSerialise_ts_1 = require("./plutusSerialise.js");
/** The saved point is no longer on the source's canonical chain. */
class HistoryIntersectionError extends Error {
}
exports.HistoryIntersectionError = HistoryIntersectionError;
/** A pinned source snapshot rolled back while it was being consumed. */
class HistorySnapshotChangedError extends Error {
}
exports.HistorySnapshotChangedError = HistorySnapshotChangedError;
const stored = (data) => lucid_1.Data.to(data, undefined, { canonical: true });
const ref = (value) => `${value.txHash}#${value.outputIndex}`;
...
function state(datum, deployment) {
    ...
    if (stored(height) !== stored(clientFields[6]) ||
    ...
    const record = (0, consensusHistory_ts_1.recordFromConstr)(new lucid_1.Constr(0, [token, height, value, time, processedHeight]));
    if (record.clientToken.policyId !== deployment.clientToken.policyId ||
...
function mapValue(data, key) {
    ...
    const entries = data.map.filter(({ k }) => k instanceof cbor_1.CborUInt && k.num === key);
    if (entries.length > 1)
        throw new Error("duplicate transaction CBOR field");
    return entries[0]?.v;
}
function transaction(evidence) {
    ...
    let bodyBytes;
    try {
        const { parsed, offset } = cbor_1.Cbor.parseLazyWithOffset(evidence.cbor);
        if (offset !== evidence.cbor.length / 2) {
            throw new Error("trailing transaction bytes");
        }
        if (parsed instanceof cbor_1.LazyCborArray && parsed.array.length === 4) {
            bodyBytes = parsed.array[0];
            const full = lucid_1.CML.Transaction.from_cbor_hex(evidence.cbor);
            envelopeValid = full.is_valid();
            decoded = full.body();
        }
        else if (parsed instanceof cbor_1.LazyCborMap) {
            bodyBytes = node_buffer_1.Buffer.from(evidence.cbor, "hex");
            decoded = lucid_1.CML.TransactionBody.from_cbor_bytes(bodyBytes);
        }
        else
            throw new Error("not a transaction or body");
    }
    catch {
    ...
    if (node_buffer_1.Buffer.from((0, blake2b_1.blake2b)(bodyBytes, { dkLen: 32 })).toString("hex") !==
        evidence.txHash) {
        throw new Error("historical transaction hash does not match CBOR");
    }
    if (envelopeValid === undefined && evidence.valid !== true) {
        throw new Error("history requires full transaction CBOR or independently verified canonical validity for a body");
    }
    ...
    const rawOutputs = mapValue(cbor_1.Cbor.parse(bodyBytes), 1n);
    if (!(rawOutputs instanceof cbor_1.CborArray)) {
...
function validateHistoryBootstrap(evidence, clientToken, outputIndex, stateAddress) {
    natural(outputIndex, "bootstrap output index");
    const { tx } = transaction(evidence);
    const quantity = tx.mint()?.get(lucid_1.CML.ScriptHash.from_hex(clientToken.policyId), lucid_1.CML.AssetName.from_hex(clientToken.name));
    if (quantity !== 1n) {
        throw new Error("bootstrap transaction must mint exactly the configured client NFT");
    }
    if (outputIndex >= tx.outputs().len()) {
        throw new Error("bootstrap output index is unavailable");
    }
    const output = (0, lucid_1.coreToUtxo)(lucid_1.CML.TransactionUnspentOutput.new(lucid_1.CML.TransactionInput.new(lucid_1.CML.TransactionHash.from_hex(evidence.txHash), BigInt(outputIndex)), tx.outputs().get(outputIndex)));
    if (output.assets[clientToken.policyId + clientToken.name] !== 1n ||
        (stateAddress !== undefined && output.address !== stateAddress)) {
        throw new Error("bootstrap NFT output does not match the deployment");
    }
}
function outputs(tx, evidence, unit, rawOutputs) {
    const result = [];
    const txOutputs = tx.outputs();
    for (let i = 0; i < txOutputs.len(); i++) {
        const utxo = (0, lucid_1.coreToUtxo)(lucid_1.CML.TransactionUnspentOutput.new(lucid_1.CML.TransactionInput.new(lucid_1.CML.TransactionHash.from_hex(evidence.txHash), BigInt(i)), txOutputs.get(i)));
        if (utxo.assets[unit] !== undefined) {
            ...
            if (!(datum instanceof cbor_1.CborArray) || datum.array.length !== 2 ||
                !(datum.array[0] instanceof cbor_1.CborUInt) || datum.array[0].num !== 1n ||
                !(datum.array[1] instanceof cbor_1.CborTag) || datum.array[1].tag !== 24n ||
                !(datum.array[1].data instanceof cbor_1.CborBytes))
                throw new Error("historical state requires original inline datum CBOR");
            utxo.datum = node_buffer_1.Buffer.from(datum.array[1].data.bytes).toString("hex");
            result.push(utxo);
        }
    }
    return result;
...
class ConsensusHistoryRecovery {
    #db;
    #tree;
    #deployment;
    #unit;
    ...
    constructor(path, deployment) {
        ...
        const suffix = new TextDecoder("utf-8", { fatal: true }).decode(Uint8Array.from(deployment.clientToken.name.slice(48).match(/../g), (pair) => parseInt(pair, 16)));
      ...
      `);
            ...
            this.#db.prepare("INSERT OR IGNORE INTO history_deployment VALUES (1, ?)").run(config);
            this.#tree = new incrementalIbcTree_ts_1.IncrementalIbcTree(this.#db);
        }
        catch (error) {
            this.#db.close();
            throw error;
    ...
    async recover(source, options = {}) {
        this.open();
        if (this.#recovering)
            throw new Error("history recovery already running");
        const maxPasses = natural(options.maxPasses ?? 8, "recovery pass limit");
        if (maxPasses < 1)
            throw new Error("recovery pass limit must be positive");
        ...
        try {
            ...
            while (passes < maxPasses) {
                ...
                try {
                    for await (const evidence of source.transactions(point)) {
                        if (point && !previous &&
                            ...
                            throw new Error("history source did not resume at the saved point");
                        }
                        if (previous &&
                        ...
                        if (candidates.length === 0) {
                            ...
                            if (resuming || (current && this.spends(tx, current))) {
                                throw new Error("unsupported history transition without state output");
                            }
                            continue;
                        }
                        ...
                        this.atomic(() => {
                            ...
                            if (resuming) {
                                ...
                                if ((this.tip()?.sequence ?? 0) !== sequence - 1) {
                                    throw new Error("history database changed during recovery, retry");
                                }
                    ...
                    if (error instanceof HistoryIntersectionError && checkpoint && !previous) {
                        this.atomic(() => {
                            this.checkCache();
                ...
                if (!previous) {
                    throw new Error("history is missing the bootstrap or saved transaction");
                }
                const tip = this.tip();
                if (!tip) {
            ...
            throw new Error("history has not caught up to the live state, progress saved; retry");
        }
        finally {
            this.#recovering = false;
        }
    }
    witness(token, height) {
        this.open();
        if (!this.#ready || this.#recovering) {
            throw new Error("history has not been matched to the live state");
        }
        if (token.policyId !== this.#deployment.clientToken.policyId ||
        ...
        try {
            ...
            if (this.#tree.getRoot() !== this.#publishedRoot ||
                ...
                throw new Error("history changed since live state validation, recover again");
            }
            checkingCache = false;
            const key = (0, consensusHistory_ts_1.consensusHistoryKey)(token, height);
            ...
            const witness = {
                ...
                key,
                value,
                record: (0, consensusHistory_ts_1.decodeConsensusHistoryRecord)(value),
                siblings: this.#tree.getSiblings(key),
            };
            if ((0, consensusHistory_ts_1.consensusHistoryKey)(witness.record.clientToken, witness.record.height) !== key ||
                (0, consensusHistory_ts_1.encodeConsensusHistoryRecord)(witness.record) !== value ||
                !(0, incrementalIbcTree_ts_1.verifyIbcTreeWitness)(key, value, witness.siblings, witness.root)) {
                throw new Error("history cache is inconsistent, rebuild from chain history");
            }
            this.#db.exec("COMMIT");
            return witness;
        }
    ...
    current() {
        return this.readReady(() => {
            ...
            const latest = this.recordRow((0, consensusHistory_ts_1.consensusHistoryKey)(snapshot.record.clientToken, snapshot.record.height));
            if (!latest)
                throw new Error("latest history record is missing");
    ...
    insertionWitness() {
        return this.readReady(() => {
            ...
            const key = (0, consensusHistory_ts_1.consensusHistoryKey)(record.clientToken, record.height);
            const value = (0, consensusHistory_ts_1.encodeConsensusHistoryRecord)(record);
            ...
            if (!(0, incrementalIbcTree_ts_1.verifyIbcTreeWitness)(key, "", siblings, root)) {
            ...
            try {
                ...
                this.#db.exec("ROLLBACK TO history_insertion; RELEASE history_insertion");
            }
        });
    }
    ...
    async *records() {
        ...
        while (true) {
            const entry = this.readReady(() => {
                ...
                return this.#db.prepare("SELECT * FROM history_records WHERE introduced_sequence > ? ORDER BY introduced_sequence LIMIT 1").get(sequence);
            });
            if (!entry)
                return;
            sequence = entry.introduced_sequence;
            const record = (0, consensusHistory_ts_1.decodeConsensusHistoryRecord)(entry.record);
            yield {
                record,
                consensusValue: entry.consensus_value,
                archived: entry.key !==
                    (0, consensusHistory_ts_1.consensusHistoryKey)(current.record.clientToken, current.record.height),
            };
        }
    }
    /** Numeric height ordering follows the validated increasing client lineage. */
    async heights(options = {}) {
        ...
        const afterKey = options.after &&
            (0, consensusHistory_ts_1.consensusHistoryKey)(this.#deployment.clientToken, options.after);
        return await Promise.resolve(this.readReady(() => {
            ...
            const rows = this.#db.prepare("SELECT record FROM history_records WHERE introduced_sequence > ? ORDER BY introduced_sequence LIMIT ?").all(cursor?.introduced_sequence ?? 0, limit);
            return rows.map((item) => (0, consensusHistory_ts_1.decodeConsensusHistoryRecord)(item.record).height);
        }));
    }
    close() {
        if (this.#recovering) {
    ...
    open() {
        if (this.#closed)
            throw new Error("history database is closed");
    }
    recordRow(key) {
        return this.#db.prepare("SELECT * FROM history_records WHERE key = ?").get(key);
    }
    readReady(operation) {
        this.open();
    ...
    row(sequence) {
        return this.#db.prepare("SELECT * FROM history_journal WHERE sequence = ?").get(sequence);
    }
    tip() {
        return this.#db.prepare("SELECT * FROM history_journal ORDER BY sequence DESC LIMIT 1").get();
    }
    point(row) {
        return {
            txHash: row.tx_hash,
    ...
    checkCache() {
        ...
        if (version !== this.#checkedDataVersion) {
            ...
            const latestKey = latestRecord &&
                (0, consensusHistory_ts_1.consensusHistoryKey)(latestRecord.clientToken, latestRecord.height);
            for (const row of this.#db.prepare("SELECT * FROM history_records ORDER BY introduced_sequence").iterate()) {
                ...
                if ((0, consensusHistory_ts_1.consensusHistoryKey)(original.record.clientToken, original.record.height) !== row.key ||
                    (0, consensusHistory_ts_1.encodeConsensusHistoryRecord)(original.record) !== row.record ||
                    original.consensusValue !== row.consensus_value) {
                    throw new Error("history record cache is inconsistent, rebuild from chain history");
                }
                const archived = this.#tree.get(row.key);
                if (!latest)
                    throw new Error("history record has no checkpoint");
                if (row.key !== latestKey && archived !== row.record) {
                    throw new Error("history record is not authenticated by the private root; rebuild from chain history");
                }
            }
            for (const leaf of this.#db.prepare("SELECT key, value FROM ibc_tree_leaves")
                .iterate()) {
                const key = node_buffer_1.Buffer.from(leaf.key).toString("utf8");
            ...
            if (latestKey && !this.recordRow(latestKey)) {
                throw new Error("latest history record is missing from the index; rebuild from chain history");
            }
        ...
        if (this.#tree.getRoot() !== expected) {
            throw new Error("history cache checkpoint is inconsistent, rebuild from chain history");
        }
    }
    checkOutput(output) {
    ...
    spends(tx, previous) {
        ...
        for (let i = 0; i < inputs.len(); i++) {
            ...
            if (input.transaction_id().to_hex() === previous.tx_hash &&
    ...
    apply(sequence, evidence, tx, output) {
        ...
        const undo = [];
        const put = (key, value, insert = false) => {
            const old = this.#tree.get(key);
            if (insert && old !== undefined) {
                throw new Error("history transition would overwrite an existing record");
            }
        ...
        if (sequence === 1) {
            ...
            if (next.record.height.revisionHeight === old.record.height.revisionHeight) {
                if ((0, consensusHistory_ts_1.encodeConsensusHistoryRecord)(next.record) !==
        ...
        const key = (0, consensusHistory_ts_1.consensusHistoryKey)(next.record.clientToken, next.record.height);
        const existingRecord = this.recordRow(key);
        if (existingRecord) {
            if (existingRecord.record !== (0, consensusHistory_ts_1.encodeConsensusHistoryRecord)(next.record)) {
                throw new Error("history record would be overwritten");
            }
        }
        else {
            this.#db.prepare("INSERT INTO history_records VALUES (?, ?, ?, ?)").run(key, (0, consensusHistory_ts_1.encodeConsensusHistoryRecord)(next.record), next.consensusValue, sequence);
        }
        this.#db.prepare(`
      INSERT INTO history_journal VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(sequence, evidence.txHash, output.outputIndex, evidence.blockHash, evidence.blockHeight, evidence.slot, evidence.transactionIndex, output.datum, JSON.stringify(undo));
    }
    rewind(sequence) {
        const rows = this.#db.prepare("SELECT * FROM history_journal WHERE sequence > ? ORDER BY sequence DESC").all(sequence);
        for (const row of rows) {
            const undo = JSON.parse(row.undo);
            for (const [key, value] of undo.reverse()) {
                this.#tree.set(key, value ?? "");
            }
            this.#db.prepare("DELETE FROM history_journal WHERE sequence = ?").run(row.sequence);
        }
        this.#db.prepare("DELETE FROM history_records WHERE introduced_sequence > ?").run(sequence);
    }
}
exports.ConsensusHistoryRecovery = ConsensusHistoryRecovery;
