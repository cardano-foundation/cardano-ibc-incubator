...
exports.createUnsignedSendPacketEscrowTx = createUnsignedSendPacketEscrowTx;
function createUnsignedSendPacketEscrowTx(dependencies, dto) {
    const internalError = dependencies.internalError ?? ((message) => new Error(message));
    ...
    if (!dto.transferModuleReferenceUtxo || (!dto.transferEscrowUtxo && (!dto.transferEscrowShardTokenUnit ||
        !dto.encodedMintTransferEscrowShardRedeemer ||
        !dto.encodedUpdatedTransferModuleDatum))) {
        throw internalError('Transfer module reference UTxO, shard token, and shard mint redeemer are required to create an escrow shard');
    }
    ...
    if (dto.transferEscrowUtxo) {
        ...
        tx
            ...
            .pay.ToContract(dependencies.transferModuleRootAddress, { kind: 'inline', value: dto.encodedUpdatedTransferModuleDatum }, dto.transferModuleReferenceUtxo.assets);
    }
    ...
    for (const [unit, amount] of Object.entries(updatedAssets)) {
        if (amount === 0n)
            delete updatedAssets[unit];
    }
    if ((updatedAssets[dto.denomToken] ?? 0n) > 0n ||
        Object.keys(updatedAssets).some((unit) => unit !== 'lovelace')) {
        tx.pay.ToContract(dto.transferModuleAddress, { kind: 'inline', value: dto.encodedTransferEscrowDatum }, updatedAssets);
    }
    return tx;
}
