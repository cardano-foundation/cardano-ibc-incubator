...
func (res *clientInfo) parseClientAttribute(log *zap.Logger, attr sdk.Attribute) {
	...
	case clienttypes.AttributeKeyConsensusHeight:
		revisionHeight, err := strconv.ParseUint(attr.Value, 10, 64)
		if err != nil {
			log.Error("Error parsing client consensus height revision height",
				zap.Error(err),
			)
		}
		res.consensusHeight.RevisionHeight = revisionHeight
...
const (
	RegisterSPO   = "register"
	UnRegisterSPO = "unregister"
)
...
func (receiver *spoInfo) parseAttrs(log *zap.Logger, attrs []sdk.Attribute) {}
func (receiver spoInfo) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	return nil
}

// client icq info
// TODO: review client icq info
