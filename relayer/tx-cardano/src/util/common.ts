...
// import { PROJECID, BLOCKFROST_URL, NETWORK } from "./constant";
...
export async function InitLucidKupmios() {
    ...
    const lucid = await Lucid.new(
        new Kupmios(
            process.env.KUPO_DEV_NET,
            process.env.OGMIOS_DEV_NET,
        ),
        network,
    );
    return lucid
...
export function GetPrivateKeyDefault() {
