...
function deriveVoucherPresentation(fullDenom: string, baseDenom: string) {
  ...
  const normalizedSymbol = /^u[a-z0-9]+$/i.test(baseLabel) && baseLabel.length > 1
...
function computeIbcDenomHash(fullDenom: string): string {
  return sha256(fullDenom).toLowerCase();
}
...
function mapVoucherTrace(
  ...
  return {
    ...
    voucherPolicyId,
    ibcDenomHash: computeIbcDenomHash(fullDenom),
...
function parseCardanoAssetId(assetId: string): {
  ...
  if (!/^[0-9a-f]+$/i.test(normalized)) {
    throw new Error('"assetId" must be a hex-encoded Cardano asset unit or "lovelace"');
  }
...
function getTraceRegistry(manifest: BridgeManifest) {
  if (!manifest.trace_registry) {
    throw new Error('Bridge manifest does not include Cardano trace-registry config');
  }
...
function getVoucherPolicyId(manifest: BridgeManifest): string {
  ...
  if (!policyId) {
    throw new Error('Bridge manifest does not include the Cardano voucher mint policy');
  }
  return policyId;
}

function getKupmiosEndpoints() {
  const [kupoUrl, ogmiosUrl] = KUPMIOS_URL.split(',').map((value) => value.trim());
  if (!kupoUrl || !ogmiosUrl) {
    throw new Error(
      'NEXT_PUBLIC_KUPMIOS_URL must be set to "<kupo-url>,<ogmios-url>" for on-chain trace-registry lookups.',
    );
  }
...
async function getBridgeManifest(): Promise<BridgeManifest> {
  if (!bridgeManifestPromise) {
    bridgeManifestPromise = fetch(CARDANO_BRIDGE_MANIFEST_URL).then(
      async (response) => {
        if (!response.ok) {
          throw new Error(
            `Failed to load Cardano bridge manifest from ${CARDANO_BRIDGE_MANIFEST_URL} (${response.status})`,
          );
        }
...
async function getKupmiosProvider() {
  if (!kupmiosProviderPromise) {
    kupmiosProviderPromise = (async () => {
      ...
      ) as Promise<typeof import('@lucid-evolution/lucid')>);
      const { kupoUrl, ogmiosUrl } = getKupmiosEndpoints();

      return {
        Lucid,
        provider: new Lucid.Kupmios(kupoUrl, ogmiosUrl) as unknown as KupmiosProvider,
      };
    })();
  }
...
async function loadRegistryContext(): Promise<LoadedRegistryContext> {
  ...
  const directoryUnit = `${registry.directory.policy_id}${registry.directory.token_name}`
    .toLowerCase();
  const directoryUtxo = await provider.getUtxoByUnit(directoryUnit);

  if (!directoryUtxo?.datum) {
    throw new Error('Unable to load the canonical Cardano trace-registry directory UTxO');
  }

  const decoded = decodeTraceRegistryDatum(directoryUtxo.datum, Lucid);
  if (!('Directory' in decoded)) {
    throw new Error('Trace-registry directory witness does not carry a directory datum');
  }
...
async function loadBucketShards(
  ...
  return Promise.all(
    uniqueTokenNames.map(async (tokenName) => {
      const unit = `${context.registry.shard_policy_id}${tokenName}`.toLowerCase();
...
async function findVoucherEntryByHash(hash: string): Promise<TraceRegistryEntry | null> {
...
export async function lookupCardanoAssetDenomTraceFromRegistry(
  ...
  if (parsed.policyId !== voucherPolicyId) {
    return buildNativeAssetTrace(parsed.assetId, parsed.assetId, parsed.assetId);
  }
