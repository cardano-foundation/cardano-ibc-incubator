...
export IGNITE_DEFAULT_HOME="/root/.cardano-entrypoint"
...
mkdir -p "$(dirname "${CARDANO_ENTRYPOINT_HOME}")"
rm -rf "${CARDANO_ENTRYPOINT_HOME}"
cp -a "${IGNITE_DEFAULT_HOME}" "${CARDANO_ENTRYPOINT_HOME}"
...
mkdir -p "${CARDANO_ENTRYPOINT_HOME}"
jq -n \
    ...
    --arg cardano_entrypointd_sha256 "${binary_hash}" \
    ...
    '{
      ...
      cardano_entrypointd_sha256: $cardano_entrypointd_sha256,
