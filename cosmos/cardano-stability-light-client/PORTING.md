...
4. Keep the protobuf package and type URLs unchanged so relayer messages still use `/ibc.lightclients.stability.v1.*`.
5. Wire the app by registering the concrete types with the interface registry and adding `stability.NewAppModule()` in the Injective app module list, following the `07-tendermint` module shape in `ibc-go/v8`.
6. Ensure the IBC client params allow `08-cardano-stability`; on restricted networks this requires governance or genesis/config changes in addition to the binary change.
7. Re-run client creation against Injective testnet after a binary with this module is deployed. Without that chain upgrade, Injective nodes will continue rejecting `/ibc.lightclients.stability.v1.ClientState` as an unresolved type URL.
