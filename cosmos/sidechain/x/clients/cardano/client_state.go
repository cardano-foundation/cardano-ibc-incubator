...
func (cs ClientState) IsExpired(latestTimestamp, now time.Time) bool {
	expirationTime := latestTimestamp.Add(time.Duration(cs.TrustingPeriod))
...
func (cs ClientState) VerifyMembership(
	...
	// get utxo
	utxo := MustUnmarshalUTXO(clientStore.Get(proof))
	utxoBz, _ := hex.DecodeString(utxo.DatumHex)

	// compare value
	if !bytes.Equal(utxoBz, value) {
		return errorsmod.Wrap(clienttypes.ErrFailedMembershipVerification, "please ensure the proof is match")
	}
...
func (cs ClientState) VerifyNonMembership(
	...
	return merkleProof.VerifyNonMembership([]*ics23.ProofSpec{}, nil, merklePath)
}
