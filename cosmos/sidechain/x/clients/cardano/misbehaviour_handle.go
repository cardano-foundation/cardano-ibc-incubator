...
func (cs *ClientState) verifyMisbehaviour(ctx sdk.Context, clientStore storetypes.KVStore, cdc codec.BinaryCodec, misbehaviour *Misbehaviour) error {
	...
	if err := checkMisbehaviourBlockData(
		cs, cardanoConsensusState1, misbehaviour.BlockData1, ctx.BlockTime(),
	); err != nil {
		return errorsmod.Wrap(err, "verifying Header1 in Misbehaviour failed")
	}
	if err := checkMisbehaviourBlockData(
		cs, cardanoConsensusState2, misbehaviour.BlockData1, ctx.BlockTime(),
	); err != nil {
...
func checkMisbehaviourBlockData(
	clientState *ClientState, consState *ConsensusState, blockData *BlockData, currentTimestamp time.Time,
) error {
	return nil
	//tmTrustedValset, err := tmtypes.ValidatorSetFromProto(blockData.TrustedValidators)
	//if err != nil {
	//	return errorsmod.Wrap(err, "trusted validator set is not cardano validator set type")
	//}
	//
	//tmCommit, err := tmtypes.CommitFromProto(blockData.Commit)
	//if err != nil {
	//	return errorsmod.Wrap(err, "commit is not cardano commit type")
	//}
	//
	//// check the trusted fields for the blockData against ConsensusState
	...
	//	chainID, _ = clienttypes.SetRevisionNumber(chainID, blockData.GetHeight().GetRevisionNumber())
	//}
	//
	...
	//	chainID, tmCommit, clientState.TrustLevel.ToCardano(),
	//); err != nil {
	//	return errorsmod.Wrapf(clienttypes.ErrInvalidMisbehaviour, "validator set in blockData has too much change from trusted validator set: %v", err)
	//}
	//return nil
}
