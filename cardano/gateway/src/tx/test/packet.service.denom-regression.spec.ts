...
describe('PacketService denom regression coverage', () => {
  it('resolves ibc/<hash> to canonical denom and uses burn path packet/module denoms', async () => {
    ...
    const lucidServiceMock = {
      ...
      decodeDatum: jest.fn(),
      resolveClientAtHeights: jest.fn(async (clientUtxo: { datum: string }): Promise<unknown> => ({ clientUtxo, clientDatum: await lucidServiceMock.decodeDatum(clientUtxo.datum, 'client'), historyUtxos: [] })),
