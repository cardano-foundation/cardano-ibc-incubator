...
const record: ConsensusStateDatum = {
  clientToken: token,
  height: height(1n),
...
async function context() {
  ...
  const clientUtxo: Lucid.UTxO = {
    txHash: '01'.repeat(32),
    outputIndex: 0,
    address: 'client',
    assets: { [token.policyId + token.name]: 1n },
    datum: await encodeClientDatum(client, Lucid),
  };
  const deployment: any = {
    hostStateNFT: token,
    validators: {
      mintClientStt: { scriptHash: token.policyId },
      spendClient: { address: 'client' },
      hostStateStt: { address: 'host' },
      recoverClient: { address: 'support' },
    },
  };
...
describe('Lucid proof-backed consensus history', () => {
  ...
  it.each(['address', 'token'] as const)(
    'rejects a client with the wrong %s before querying history',
    async (field) => {
      const { service, clientUtxo, history } = await context();
  ...
  it('updates only the HostState and client outputs and invokes the bound support script', async () => {
    ...
    await service.createUnsignedRecoverClientTransaction(
      clientUtxo,
      'host',
      clientUtxo,
      'spend',
    ...
    expect(builder.readFrom).toHaveBeenLastCalledWith([
      refs.hostStateStt,
      refs.spendClient,
      refs.recoverClient,
      substitute,
