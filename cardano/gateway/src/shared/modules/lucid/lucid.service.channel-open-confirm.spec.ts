...
const deploymentConfig = {
  ...
  validators: {
    ...
    mintChannelStt: buildValidator('mint-channel', 14),
    mintClientStt: buildValidator('mint-client', 15),
    mintConnectionStt: buildValidator('mint-connection', 16),
    mintVoucher: buildValidator('mint-voucher', 17),
    verifyProof: {
      ...buildValidator('verify-proof', 18),
      scriptHash: 'verify-proof-policy-id',
    },
    hostStateStt: buildValidator('host-state', 19),
  },
  modules: {
    mock: {
...
describe('LucidService channel open confirm wiring', () => {
  ...
  it('uses the confirm and verify-proof refs when building ChannelOpenConfirm transactions', () => {
    ...
    service.createUnsignedChannelOpenConfirmTransaction({
      ...
      mockModuleUtxo: { txHash: 'mock-utxo', outputIndex: 0, assets: { lovelace: 2_000_000n } } as any,
      encodedSpendChannelRedeemer: 'encoded-channel-redeemer',
      encodedSpendMockModuleRedeemer: 'encoded-mock-redeemer',
