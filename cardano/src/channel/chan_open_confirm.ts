...
export const chanOpenConfirm = async (
  lucid: Lucid,
  ...
  if (channelDatum.state.channel.state !== "TryOpen") {
    throw new Error("ChanOpenConfirm to channel not in TryOpen state");
  }
  ...
  const spendChannelRedeemer: SpendChannelRedeemer = {
    ChanOpenConfirm: {
      proof_ack: fromText(op.proofAck),
  ...
  const txHash = await submitTx(ChanOpenConfirmTx);
  console.log("ChanOpenConfirm tx submitted with hash:", txHash);
  await lucid.awaitTx(txHash);
  console.log("ChanOpenConfirm tx completed");
...
const main = async () => {
  ...
  await chanOpenConfirm(lucid, deploymentInfo, op);
};
