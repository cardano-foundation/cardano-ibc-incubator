...
import { ChannelCounterparty } from "../types/channel/counterparty.ts";
import { Order } from "../types/channel/order.ts";
import {
  ChannelDatum,
  ChannelDatumState,
} from "../types/channel/channel_datum.ts";
...
export const chanOpenTry = async (
  lucid: Lucid,
  deploymentInfo: DeploymentTemplate,
  op: Operator,
) => {
  console.log("Channel Open Try");
  ...
  const channel: Channel = {
    state: "TryOpen",
    counterparty: {
      port_id: fromText(op.counterparty.port_id),
      channel_id: fromText(op.counterparty.channel_id),
    },
  ...
  const chanOpenTryTx = lucid.newTx()
  ...
  const txHash = await submitTx(chanOpenTryTx);
  console.log("ChanOpenTry tx submitted with hash:", txHash);
  await lucid.awaitTx(txHash);
  console.log("ChanOpenTry tx completed");
...
const main = async () => {
  ...
  await chanOpenTry(lucid, deploymentInfo, op);
};
