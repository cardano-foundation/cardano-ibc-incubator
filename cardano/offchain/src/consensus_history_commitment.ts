...
// Prototype storage only: this does not verify Tendermint headers or authorize
...
interface HistoryView {
  ...
  readonly ready: Promise<DeploymentIbcTree>;
}
...
export class ConsensusHistoryCommitment {
  ...
  private currentView(): HistoryView {
    if (!this.#view) {
      ...
      for (const [key, value] of records) tree.set(key, value);
      this.#view = { records, ready: tree.getRoot().then(() => tree) };
    }
    return this.#view;
  }
  ...
  private async makeWitness(
    ...
    return {
      ...
      siblings: await tree.getSiblings(key),
    };
  }
}
