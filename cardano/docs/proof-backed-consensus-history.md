...
The pinned Hermes signer recognizes `proof-backed-v1` and checks the update withdrawal and new recovery redeemer against the pinned scripts and requested clients. Its tests use update and recovery transactions accepted by the emulator with separate spending and collateral inputs. The Gateway's automatic input selection does not yet guarantee that separation, which Hermes requires. Live Yaci recovery, a long-history recovery-time measurement and independent deployment testing also remain required before production use. The existing 64-bit tree path bound and validity-bound processing time are unchanged.
