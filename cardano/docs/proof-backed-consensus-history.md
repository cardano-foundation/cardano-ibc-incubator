...
The index checks every predecessor and resulting root against transaction outputs. Processing metadata comes from the consumed datum, never recovery time. Public Data encodings are preserved using a serialization adapter checked against Aiken's evaluator. Tree changes and rollback records commit together only after matching the independently read live NFT output before and after replay. Tests cover interrupted replay, replacement forks, restart validation and missing or corrupt history. The database is disposable, but some independently retained source of historical transactions remains necessary.
