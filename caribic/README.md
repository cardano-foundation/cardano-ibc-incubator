...
- The Hermes daemon's client refresh should run continuously. In the July 24, 2026 failure documented in [#552](https://github.com/cardano-foundation/cardano-ibc-incubator/issues/552), a catch-up update spanning Cardano heights 4,970,800 to 4,971,057 encoded to 3,073,891 bytes. Although that was below Injective's 4,194,304-byte (4 MiB) consensus block limit, the tested public nodes rejected it before inclusion because it exceeded their effective 1,048,576-byte per-transaction mempool limit. The amount of downtime before this happens varies with witness sizes and available HostState anchors; when no valid intermediate anchor exists, recovery through the current flow requires a new client, connection, and channel (`caribic setup route` reuses an existing channel, so a rebuild currently requires driving `hermes create client` / `create connection` / `create channel` manually).
