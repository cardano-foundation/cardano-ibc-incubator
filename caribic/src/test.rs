...
async fn verify_services_running(project_root: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !cardano_running {
        missing_services.push("Cardano node".to_string());
    } else {
    ...
    if !gateway_running {
        missing_services.push("Gateway".to_string());
    } else {
    ...
    .await;
    if !pfc_running {
        missing_services.push("Packet-forwarding chain (Cosmos) on :26657".to_string());
    } else {
        verbose("   Packet-forwarding chain is running");
    }

    // Regression guard: ensure the Cosmos Entrypoint faucet is not publicly exposed.
    //
    // Ignite starts a faucet server on port 4500 during `ignite chain serve`.
    // We want that port to either:
    //   - not be published at all, or
    //   - be published only to loopback (127.0.0.1 / ::1).
    if let Err(message) = ensure_entrypoint_faucet_not_public(project_root) {
        missing_services.push(message);
    } else {
        verbose("   Entrypoint faucet port is not publicly exposed");
    }
    ...
    if !mithril_running {
        missing_services.push("Mithril aggregator on :8080".to_string());
    } else {
        ...
        if !stake_distributions_ready {
            missing_services.push("Mithril stake distributions (not ready)".to_string());
        } else {
        ...
        if !tx_snapshots_ready {
            missing_services.push("Mithril Cardano transaction snapshots (not ready)".to_string());
        } else {
...
fn ensure_entrypoint_faucet_not_public(project_root: &Path) -> Result<(), String> {
    let compose_path = project_root.join("cosmos/docker-compose.yml");
    let compose = std::fs::read_to_string(&compose_path).map_err(|e| {
        format!(
            "Unable to validate faucet exposure because '{}' could not be read: {}",
    ...
    for (idx, raw_line) in compose.lines().enumerate() {
        // Keep original line for error context but strip comments and whitespace for parsing.
        let line = raw_line.split('#').next().unwrap_or("").trim();
        if line.is_empty() || !line.starts_with('-') {
            continue;
        }

        let mut value = line.trim_start_matches('-').trim();
        if value.is_empty() {
            continue;
        }
        ...
        // Remove optional protocol suffix like "/tcp".
        let value = value.split('/').next().unwrap_or(value).trim();
        if !value.contains("4500") {
            continue;
        }

        // Parse docker-compose short port syntax:
        // - "127.0.0.1:4500:4500"
        // - "4500:4500"
        // - "4500"
        // Also handle IPv6 loopback "[::1]:4500:4500".
        let parsed = if value.starts_with('[') {
            ...
            match parts.len() {
                ...
                1 => Some((None, String::new(), parts[0].to_string())),
                _ => None,
            }
        };

        let Some((host_ip, _host_port, container_port)) = parsed else {
            continue;
        };

        if container_port != "4500" {
            continue;
        }

        let is_loopback_only = matches!(host_ip.as_deref(), Some("127.0.0.1") | Some("::1"));
        if !is_loopback_only {
            offenders.push((idx + 1, raw_line.to_string()));
        }
    }
    ...
    Err(format!(
        "Cosmos Entrypoint faucet appears publicly exposed (port 4500 should be unpublished or bound to 127.0.0.1/::1) in '{}':{}",
        compose_path.display(),
        details
    ))
}
