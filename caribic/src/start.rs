...
use crate::utils::{
    ...
    get_slots_in_epoch, get_yaci_state, unzip_file, wait_for_health_check, wait_until_file_exists,
...
pub async fn start_db_sync(project_root_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let cardano_dir = project_root_path.join("chains/cardano");
    if config::get_config().cardano.services.db_sync {
        prepare_db_sync(cardano_dir.as_path())?;
        execute_script(
...
pub async fn wait_and_start_mithril_genesis(
    ...
    let slots_per_epoch = get_slots_in_epoch().await?;
