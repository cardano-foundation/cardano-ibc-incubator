pub(super) const LOCAL_STATUS_URL: &str = "http://127.0.0.1:26658/status";

pub(super) const SOURCE_ZIP_URL: &str =
    "https://github.com/osmosis-labs/osmosis/archive/refs/tags/v30.0.1.zip";
pub(super) const TESTNET_RPC_URL: &str = "https://rpc.osmotest5.osmosis.zone";
pub(super) const TESTNET_STATUS_URL: &str = "http://127.0.0.1:26658/status";
pub(super) const TESTNET_CHAIN_ID: &str = "osmo-test-5";
pub(super) const TESTNET_MONIKER: &str = "caribic-osmosis-testnet";
pub(super) const TESTNET_HOME_DIR: &str = ".osmosisd-testnet";
pub(super) const TESTNET_PID_FILE: &str = ".caribic/osmosis-testnet.pid";
pub(super) const TESTNET_LOG_FILE: &str = ".caribic/osmosis-testnet.log";
pub(super) const TESTNET_TRUST_OFFSET: u64 = 1500;
pub(super) const TESTNET_SEEDS: &str = "bb197876fd952d245ef6377e3651c157e3d7ed81@157.245.26.231:26656,7c2b9e76be5c2142c76b429d9c29e902599ceb44@157.245.21.183:26656";
pub(super) const TESTNET_PERSISTENT_PEERS: &str = "51084fccec1c309a415e89d39e6f0881c49493ed@95.217.144.107:12556,a5c34bdd777dd418ff7152a8646fd2f31f53f8a5@46.232.248.117:2000";
pub(super) const TESTNET_RPC_LADDR: &str = "tcp://0.0.0.0:26658";
pub(super) const TESTNET_GRPC_ADDRESS: &str = "0.0.0.0:9094";
pub(super) const TESTNET_GRPC_WEB_ADDRESS: &str = "0.0.0.0:9091";
pub(super) const TESTNET_API_ADDRESS: &str = "tcp://0.0.0.0:1318";
