...
deployment = load('test-cardano-deployment', Path(__file__).resolve().parent.parent)
...
class References(unittest.TestCase):
    ...
    def test_clock_restarts_retain_offset_and_never_recompute_fixed_target(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = (Path(__file__).resolve().parents[3] / 'chains/cardano/local-clock-entrypoint').read_text()
