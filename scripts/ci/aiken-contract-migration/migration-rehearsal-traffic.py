...
def packet_observation(manifest_path, channel, **options):
    ...
    response = subprocess.check_output(['node', str(ROOT / 'scripts/ci/aiken-contract-migration/verify-migration-packet.cjs')],
...
def verify_cosmos_acknowledgement(block, index, packet, transaction):
    ...
    response = subprocess.check_output(['node', str(ROOT / 'scripts/ci/aiken-contract-migration/verify-migration-cosmos-message.cjs')],
                                       input=json.dumps(request), text=True, cwd=ROOT)
    return json.loads(response.splitlines()[-1])
...
def main():
    ...
    command = ['python3', str(ROOT / 'scripts/ci/aiken-contract-migration/migration-hermes.py'), 'run', '--runtime', str(runtime),
