...
def main():
    ...
    control = ['python3', str(ROOT / 'scripts/ci/aiken-contract-migration/migration-control.py'), '--runtime', str(runtime),
    ...
    def capture(selected_handler, output):
        return run('capture', deno + [str(ROOT / 'scripts/ci/aiken-contract-migration/capture-migration-population.ts'),
    ...
    run('conservation', deno + [str(ROOT / 'scripts/ci/aiken-contract-migration/verify-migration-population.ts'),
    ...
    for transaction in sorted(transactions):
        ...
        run('measure', ['node', str(ROOT / 'scripts/ci/aiken-contract-migration/measure-migration-transaction.cjs'),
                        str(runtime), transaction, str(report)])
        reports.append(str(report))
    funding = run('funding', ['python3', str(ROOT / 'scripts/ci/aiken-contract-migration/verify-migration-funding.py'),
