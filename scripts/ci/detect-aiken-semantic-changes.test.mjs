...
import {
  ...
  isAikenInfrastructurePath,
} from "./detect-aiken-semantic-changes.mjs";

test("treats Tendermint benchmark inputs and generated assets as infrastructure", () => {
  for (const path of [
    ...
    assert.equal(isAikenInfrastructurePath(path), true, path);
  }
});
...
test("preserves comment markers and escapes inside every quoted literal form", () => {
  ...
  assert.notEqual(
    aikenSemanticSignature(after),
    aikenSemanticSignature(before),
  );
  assert.doesNotThrow(() =>
...
test("classifies modified comments as trivia and source additions as relevant", () => {
  ...
  try {
    ...
    const gatewayTypeDir = join(
      repo,
      "cardano/gateway/src/shared/types/channel",
    );
    mkdirSync(gatewayTypeDir, { recursive: true });
    writeFileSync(
      join(gatewayTypeDir, "channel-redeemer.ts"),
      "export const value = 1;\n",
    );
    execFileSync("git", ["add", "."], { cwd: repo });
    execFileSync("git", ["commit", "-qm", "change budget encoder"], {
      cwd: repo,
    });
    ...
    writeFileSync(
      join(helperDir, "hex.ts"),
      'export const toHex = () => "00";\n',
    );
    execFileSync("git", ["add", "."], { cwd: repo });
    execFileSync("git", ["commit", "-qm", "change budget helper"], {
      cwd: repo,
    });
    ...
    const lucidServiceDir = join(
      repo,
      "cardano/gateway/src/shared/modules/lucid",
    );
